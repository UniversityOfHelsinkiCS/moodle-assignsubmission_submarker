@mod @mod_assign @assignsubmission_submissionmarker
Feature: In an assignment, limit submittable file types
  ...
  Background:
    ...
    And the following config values are set as admin:
      | filetypes | image/png;spreadsheet | assignsubmission_submissionmarker |

  @javascript
  Scenario: File types validation for an assignment
    Given the following "activities" exist:
      | activity | course | idnumber | name                 | intro                       | duedate    | assignsubmission_onlinetext_enabled | assignsubmission_submissionmarker_enabled | assignsubmission_submissionmarker_maxfiles | assignsubmission_submissionmarker_maxsizebytes |
  ...
  Scenario: Uploading permitted file types for an assignment
    Given the following "activities" exist:
      | activity | course | idnumber | name                 | intro                       | duedate    | assignsubmission_onlinetext_enabled | assignsubmission_submissionmarker_enabled | assignsubmission_submissionmarker_maxfiles | assignsubmission_submissionmarker_maxsizebytes | assignsubmission_submissionmarker_filetypes |
