...
class assignsubmission_submissionmarker_events_testcase extends advanced_testcase {
    ...
    protected function setUp() {
        ...
        $dummy = (object) array(
            ...
            'component' => 'assignsubmission_submissionmarker',
            'filearea' => ASSIGNSUBMISSION_SUBMISSIONMARKER_FILEAREA,
        ...
        $this->files = $fs->get_area_files($this->context->id, 'assignsubmission_submissionmarker', ASSIGNSUBMISSION_SUBMISSIONMARKER_FILEAREA,
    ...
    public function test_assessable_uploaded() {
        ...
        $event = reset($events);
        $this->assertInstanceOf('\assignsubmission_submissionmarker\event\assessable_uploaded', $event);
    ...
    public function test_submission_created() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\assignsubmission_submissionmarker\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
    ...
    public function test_submission_updated() {
        ...
        // We want to test the last event fired.
        $event = $events[3];
        $this->assertInstanceOf('\assignsubmission_submissionmarker\event\submission_updated', $event);
        $this->assertEquals($this->context->id, $event->contextid);
        $this->assertEquals($this->course->id, $event->courseid);
        $this->assertEquals($this->submission->id, $event->other['submissionid']);
        $this->assertEquals($this->submission->attemptnumber, $event->other['submissionattempt']);
